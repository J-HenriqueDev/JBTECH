            ...
            function initProcessarNota($) {
                ...
                function updateRowCalculations(row) {
                    ...
                    if (ultimoCusto > 0) {
                        ...
                        diffContainer.html(
                            `<span class="${color}" title="Anterior: R$ ${formatCost(ultimoCusto)} (${diffPercent.toFixed(1)}%)">${icon} ${diffPercent.toFixed(1)}%</span>`
                            );
                    } else {
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center bg-primary text-white">
                    <div>
                        <h5 class="mb-0 text-white">Processar Nota:
                            {{ $nota->numero_nfe ? 'NFe ' . $nota->numero_nfe : $nota->chave_acesso }}</h5>
                        <small>{{ $nota->emitente_nome }} | Emitida em:
                            {{ \Carbon\Carbon::parse($nota->data_emissao)->format('d/m/Y') }}</small>
                    </div>
                    <div>
                    ...
                    <form action="{{ route('notas-entrada.confirmar', $nota->id) }}" method="POST">
                        ...
                        @if (!empty($duplicatas))
                            <div class="alert alert-info m-3" role="alert">
                                ...
                                <small class="text-muted d-block mt-2">Ao confirmar a entrada, as duplicatas serão
                                    registradas em Contas a Pagar.</small>
                            </div>
                        @endif
                                        ...
                                        <tr class="item-row">
                                                ...
                                                <div class="d-flex flex-column">
                                                    ...
                                                    <div class="mb-3">
                                                        <span class="fw-bold text-primary d-block mb-1"
                                                            style="font-size: 1.05rem;">{{ $item['xProd'] }}</span>
                                                        <div class="d-flex flex-wrap gap-2 text-muted small">
                                                            <span class="xml-badge"><i class="bx bx-barcode"></i>
                                                                {{ $item['cEAN'] }}</span>
                                                            <span class="xml-badge"><i class="bx bx-box"></i>
                                                                {{ $item['uCom'] }}</span>
                                                            <span class="xml-badge"><i class="bx bx-file"></i> NCM:
                                                                {{ $item['NCM'] }}</span>
                                                        </div>
                                                    </div>

                                                    <input type="hidden" name="itens[{{ $index }}][nItem]"
                                                        value="{{ $item['nItem'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][cProd]"
                                                        value="{{ $item['cProd'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][xProd]"
                                                        value="{{ $item['xProd'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][cEAN]"
                                                        value="{{ $item['cEAN'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][NCM]"
                                                        value="{{ $item['NCM'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][CEST]"
                                                        value="{{ $item['CEST'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][CFOP]"
                                                        value="{{ $item['CFOP'] }}">
                                                    <input type="hidden" name="itens[{{ $index }}][uCom]"
                                                        value="{{ $item['uCom'] }}">
                                                    ...
                                                    <div class="bg-light p-3 rounded border">
                                                        <div class="d-flex gap-4 mb-3">
                                                            <div class="form-check">
                                                                <input class="form-check-input acao-radio" type="radio"
                                                                    name="itens[{{ $index }}][acao]" value="criar"
                                                                    id="acao-criar-{{ $index }}"
                                                                    data-index="{{ $index }}"
                                                                    {{ !$item['produto_existente'] ? 'checked' : '' }}>
                                                                <label class="form-check-label"
                                                                    for="acao-criar-{{ $index }}">
                                                            ...
                                                            <div class="form-check">
                                                                <input class="form-check-input acao-radio" type="radio"
                                                                    ...
                                                                    value="associar" id="acao-associar-{{ $index }}"
                                                                    data-index="{{ $index }}"
                                                                    {{ $item['produto_existente'] ? 'checked' : '' }}>
                                                                <label class="form-check-label"
                                                                    for="acao-associar-{{ $index }}">
                                                        ...
                                                        <div class="produto-novo-container {{ $item['produto_existente'] ? 'd-none' : '' }}"
                                                            id="produto-novo-{{ $index }}">
                                                            <label class="form-label small text-muted">Nome do Novo
                                                                Produto</label>
                                                            <input type="text" class="form-control form-control-sm"
                                                                name="itens[{{ $index }}][nome_novo]"
                                                                value="{{ $item['xProd'] }}" placeholder="Nome do Produto">
                                                            <small class="text-muted d-block mt-1"
                                                                style="font-size: 0.75rem;">
                                                                <i class="bx bx-info-circle"></i> Será criado com os dados
                                                                fiscais do XML (NCM, Unidade, EAN).
                                                            </small>
                                                        </div>

                                                        <!-- Container: Associar Existente -->
                                                        <div class="produto-associar-container {{ !$item['produto_existente'] ? 'd-none' : '' }}"
                                                            id="produto-associar-{{ $index }}">
                                                            <label class="form-label small text-muted">Buscar Produto (Nome
                                                                ou Código)</label>
                                                            <select class="select2-produto form-select"
                                                                name="itens[{{ $index }}][produto_id]"
                                                                style="width: 100%;">
                                                                @if ($item['produto_existente'])
                                                                    <option value="{{ $item['produto_existente']->id }}"
                                                                        selected>
                                                                        {{ $item['produto_existente']->nome }} - (Cód:
                                                                        {{ $item['produto_existente']->codigo_barras ?? $item['produto_existente']->id }})
                                                                    </option>
                                                                @else
                                                                    <option></option>
                                                                @endif
                                            ...
                                            <td class="align-top">
                                                <div class="mb-2">
                                                    ...
                                                    <input type="number"
                                                        class="form-control form-control-sm qtd-xml bg-light"
                                                        value="{{ $item['qCom'] }}" readonly>
                                                </div>
                                                <div class="mb-2">
                                                    <label class="small text-muted d-block"
                                                        title="Fator para multiplicar a Qtd XML. Ex: Caixa com 12 un -> Fator 12.">Fator
                                                        (Multiplicador)</label>
                                                    <input type="number"
                                                        class="form-control form-control-sm fator-conversao border-primary"
                                                        name="itens[{{ $index }}][fator]" value="1"
                                                        min="0.0001" step="0.0001">
                                                </div>
                                                <div>
                                                    <label class="small text-muted d-block fw-bold">Qtd Entrada</label>
                                                    <input type="number"
                                                        class="form-control form-control-sm qtd-sistema fw-bold"
                                                        name="itens[{{ $index }}][quantidade]" readonly>
                                                </div>
                                            </td>
                                            ...
                                            <td class="align-top">
                                                <div class="mb-2">
                                                    ...
                                                    <input type="number"
                                                        class="form-control form-control-sm ultimo-custo bg-light"
                                                        value="{{ $item['ultimo_custo'] }}" readonly>
                                                </div>
                                                <div class="mb-2">
                                                    <label class="small text-muted d-block">Custo Unit. XML</label>
                                                    <input type="number"
                                                        class="form-control form-control-sm custo-xml bg-light"
                                                        value="{{ $item['vUnCom'] }}" readonly>
                                                </div>
                                                <div class="mb-2">
                                                    <label class="small text-muted d-block fw-bold">Novo Custo
                                                        Sistema</label>
                                                    <input type="number" step="0.0001"
                                                        class="form-control form-control-sm custo-sistema fw-bold border-success"
                                                        name="itens[{{ $index }}][preco_custo]" readonly>
                                                </div>
                                            </td>
                                            ...
                                            <td class="align-top">
                                                <div class="mb-2">
                                                    ...
                                                    <input type="number" step="0.01"
                                                        class="form-control form-control-sm margem-percent"
                                                        name="itens[{{ $index }}][margem]" value="50.00">
                                                </div>
                                                <div>
                                                    ...
                                                    <div class="input-group input-group-sm">
                                                        ...
                                                        <input type="number" step="0.01"
                                                            class="form-control form-control-sm preco-venda fw-bold border-primary"
                                                            name="itens[{{ $index }}][preco_venda]">
                                                    </div>
                                                </div>
                                            </td>
