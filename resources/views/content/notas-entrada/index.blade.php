...
<div id="soneca-banner" class="alert alert-info d-flex align-items-center justify-content-between mb-4" role="alert" style="{{ $healthStats['sonecaMinutos'] > 0 ? '' : 'display: none;' }}">
    <div class="d-flex align-items-center">
        <i class="bx bx-sleep-y me-2"></i>
        <span id="soneca-text">
            💤 Robô em espera técnica. Retorno em <strong id="soneca-timer">{{ $healthStats['sonecaMinutos'] }}</strong> min.
        </span>
    </div>
    <span class="badge bg-info">SEFAZ</span>
</div>

<!-- Removido: Cards de estatísticas (movidos para Dashboard Fiscal) -->

<div class="card mb-4">
    <div class="card-body">
        <form action="{{ route('notas-entrada.baixar-por-chave') }}" method="POST">
            @csrf
            <label for="chave" class="form-label fs-5 fw-bold text-primary">Ler Código de Barras / Chave de
                Acesso</label>
            <div class="input-group input-group-lg">
                ...
                <input type="text" class="form-control" id="chave" name="chave"
                    placeholder="Aponte o leitor aqui ou digite a chave de 44 dígitos..." required autofocus>
                <button class="btn btn-primary" type="submit">Baixar Nota</button>
            </div>
            <div class="form-text mt-2"><i class="fas fa-info-circle me-1"></i> O sistema detectará automaticamente o
                "Enter" do leitor de código de barras.</div>
        </form>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkHealth() {
            fetch('{{ route('fiscal.health') }}', {
                ...
                .then(data => {
                    const banner = document.getElementById('soneca-banner');
                    const timer = document.getElementById('soneca-timer');

                    if (data.sonecaMinutos > 0) {
                        banner.style.display = 'flex';
                        if (timer) timer.textContent = data.sonecaMinutos;
                    } else {
                    ...
                    if (syncIcon) {
                        ...
                        if (btnBusca) {
                            btnBusca.disabled = (data.sonecaMinutos > 0);
                        }
                    }
                })
                .catch(err => console.error('Erro ao verificar saúde fiscal:', err));
        }

        // Inicia verificação periódica (60s)
