  ...
  <script>
      ...
      function atualizarProdutos() {
          ...
          $.ajax({
              ...
              success: function(produtos) {
                  ...
                  listaProdutos = produtos; // Armazena na variável global
      ...
      $(document).ready(function() {
          ...
          // Lógica de Adição Rápida
          const inputQuickAdd = document.getElementById('inputQuickAdd');
          const btnQuickAdd = document.getElementById('btnQuickAdd');

          if (inputQuickAdd) {
              inputQuickAdd.addEventListener('keypress', function(e) {
                  if (e.key === 'Enter') {
                      e.preventDefault();
                      processQuickAdd(this.value);
                  }
              });

              btnQuickAdd.addEventListener('click', function() {
                  processQuickAdd(inputQuickAdd.value);
              });
          }

          function processQuickAdd(val) {
              ...
              // Formato: Qtd * Código * Preço (ex: 2 * 789 * 10,00) ou Qtd * Código
              if (val.includes('*')) {
                  ...
                  if (parts.length >= 2) {
                      ...
                      if (parts.length === 3) {
                          ...
                          let precoStr = parts[2].trim().replace(',', '.');
                          precoOverride = parseFloat(precoStr);
                      }
                  }
              }

              // Procura na lista carregada
              // Tenta por ID, Código de Barras ou Código Interno
              // A listaProdutos tem: id, nome, preco_venda, codigo_barras (talvez), codigo_interno (talvez)
              // Vamos assumir que 'codigo_barras' e 'id' existem.

              let produtoEncontrado = listaProdutos.find(p =>
                  p.id == codigo ||
                  (p.codigo_barras && p.codigo_barras == codigo) ||
                  (p.codigo_interno && p.codigo_interno == codigo)
              );

              if (produtoEncontrado) {
                  let valorUnitario = parseFloat(produtoEncontrado.preco_venda) || 0;

                  // Se houve override de preço via código de barras
                  if (precoOverride !== null && !isNaN(precoOverride)) {
                      valorUnitario = precoOverride;
                  }
                  ...
                  const originalPlaceholder = inputQuickAdd.placeholder;
                  inputQuickAdd.placeholder = "Produto adicionado!";
                  setTimeout(() => inputQuickAdd.placeholder = originalPlaceholder, 1500);
              } else {
                  // Tenta buscar via AJAX se não encontrou na lista local (limitada a 50 itens)
                  buscarProdutoAjax(codigo, qtd, precoOverride);
              }
          }

          function buscarProdutoAjax(codigo, qtd, precoOverride) {
              const btnQuickAdd = document.getElementById('btnQuickAdd');
              const originalBtnText = btnQuickAdd.innerHTML;
              btnQuickAdd.innerHTML = '<i class="bx bx-loader-alt bx-spin"></i> Buscando...';
              ...
              $.ajax({
                  ...
                  success: function(response) {
                      if (response.success && response.produto) {
                          ...
                          let valorUnitario = parseFloat(produto.preco_venda) || 0;
          ...
          // Atualiza o valor total ao alterar o valor unitário
          $('#valor_unitario').on('input', function() {
              const preco = parseCurrency($(this).val());
              const quantidade = parseInt($('#quantidade').val() || 1);
              $('#valor_total').val(formatCurrency(preco * quantidade));
          });
          ...
          $('#adicionarProduto').on('click', function() {
              ...
              if (!produtoId || !valorUnitario || quantidade <= 0) {
                  alert(
                      'Por favor, preencha todos os campos corretamente antes de adicionar um produto.'
                  );
                  return;
              }
          ...
          $('#emitirNFe').on('click', function() {
              ...
              $('#tabelaProdutos tbody tr').each(function() {
                  ...
                  const valorUnitario = parseCurrency($(this).find(
          ...
          $('#modalEmitirNFe').modal({
              backdrop: true,
              keyboard: true
          });
          $('#modalEmitirCobranca').modal({
              backdrop: true,
              keyboard: true
          });
      });
      ...
      $(document).ready(function() {
          ...
          $('#gerarPix').on('click', function() {
              ...
              const enviarEmail = $('#enviarEmail').is(
                  ':checked'); // Verifica se o e-mail deve ser enviado
                  ...
                  success: function(response) {
                      if (response.redirectUrl) {
                          window.location.href = response
                              .redirectUrl; // Redireciona para o PagSeguro
                      } else {
