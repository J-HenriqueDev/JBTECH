...
@section('content')
        ...
        <div class="card p-4 shadow-sm mb-4">
            ...
            <form action="{{ route('produtos.importar.chave') }}" method="POST">
                ...
                <div class="input-group mb-3">
                    <input type="text" name="chave_acesso" class="form-control form-control-lg"
                        placeholder="Digite a Chave de Acesso da NF-e (44 dígitos)" maxlength="44" required>
                    <button class="btn btn-primary" type="submit"><i class="bx bx-search"></i> Buscar e Importar</button>
                </div>
                <small class="text-muted">A busca por chave requer que a nota tenha sido emitida para o CNPJ da empresa e
                    esteja disponível na SEFAZ.</small>
            </form>
        </div>
        ...
        <div class="card p-4 shadow-sm">
            ...
            @if (!empty($productsData))
                ...
                <form action="{{ route('produtos.store') }}" method="POST">
                                    ...
                                    <tr>
                                        <td class="text-center align-middle">
                                            @if (isset($product['match_type']) && $product['match_type'] == 'existente')
                                                <span class="badge bg-info" data-bs-toggle="tooltip"
                                                    title="Produto já existe e será atualizado">Atualizar</span>
                                                <input type="hidden" name="produtos[{{ $index }}][id]"
                                                    value="{{ $product['id'] }}">
                                            @else
                                                <span class="badge bg-success" data-bs-toggle="tooltip"
                                                    title="Novo produto será criado">Novo</span>
                                            @endif
                                        </td>
                                        <td>
                                            <input type="text" name="produtos[{{ $index }}][nome]"
                                                value="{{ $product['nome'] }}" class="form-control" style="width: 100%;"
                                                required>
                                        </td>
                                        <td class="d-none d-md-table-cell">
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="produtos[{{ $index }}][preco_custo]"
                                                    ...
                                                    class="form-control" required oninput="formatCurrencyInput(this)">
                                            </div>
                                        </td>
                                        <td class="d-none d-md-table-cell">
                                            <div class="input-group">
                                        ...
                                        <td class="d-none d-md-table-cell">
                                            <input type="text" name="produtos[{{ $index }}][codigo_barras]"
                                                value="{{ $product['codigo_barras'] }}" class="form-control">
                                        </td>
                                        <td class="d-none d-md-table-cell">
                                            <input type="text" name="produtos[{{ $index }}][ncm]"
                                                value="{{ $product['ncm'] }}" class="form-control" required>
                                        </td>
                                        <td>
                                            <input type="number" name="produtos[{{ $index }}][estoque]"
                                                value="{{ $product['estoque'] }}" class="form-control" style="width: 80px;"
                                                required> <!-- Reduzido o tamanho do campo -->
                                        </td>
                                        <td>
                                            <select name="produtos[{{ $index }}][categoria_id]"
                                                class="form-control">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <label for="fornecedor_cnpj" class="form-label"><i class="bx bx-id-card"></i> CNPJ do
                                Fornecedor</label>
                        ...
                        <div class="col-md-6">
                            <label for="fornecedor_nome" class="form-label"><i class="bx bx-user"></i> Nome do
                                Fornecedor</label>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <label for="fornecedor_telefone" class="form-label"><i class="bx bx-phone"></i> Telefone do
                                Fornecedor</label>
                            <input type="text" class="form-control" name="fornecedor_telefone"
                                id="fornecedor_telefone" value="{{ $fornecedor['telefone'] ?? '' }}" required
                                oninput="formatTelefone(this)">
                        </div>
                        <div class="col-md-6">
                            <label for="fornecedor_email" class="form-label"><i class="bx bx-envelope"></i> E-mail do
                                Fornecedor</label>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const inputsPreco = document.querySelectorAll(
                'input[name*="[preco_custo]"], input[name*="[preco_venda]"]');
            inputsPreco.forEach(input => {
                input.addEventListener('input', function() {
                    const index = this.name.match(/\[(\d+)\]/)[1];
                    const precoCusto = parseFloat(document.querySelector(
                        `input[name="produtos[${index}][preco_custo]"]`).value.replace(
                        'R$ ', '').replace('.', '').replace(',', '.'));
                    const precoVenda = parseFloat(document.querySelector(
                        `input[name="produtos[${index}][preco_venda]"]`).value.replace(
                        'R$ ', '').replace('.', '').replace(',', '.'));

                    if (!isNaN(precoCusto) && !isNaN(precoVenda) && precoCusto > 0) {
                        const lucro = ((precoVenda - precoCusto) / precoCusto) * 100;
                        document.querySelector(`#lucro_percentual_${index}`).textContent =
                            `${lucro.toFixed(2)}%`;
                    } else {
