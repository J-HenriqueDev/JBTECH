                ...
                <div class="card bg-primary text-white">
                    <div class="card-body">
                        ...
                        <small class="d-block mt-1">R$
                            {{ number_format($statsCliente['total_vendas'], 2, ',', '.') }}</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                ...
                <div class="card-body">
                    ...
                    @if ($cliente->data_nascimento)
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <span
                                    class="text-muted">{{ \Carbon\Carbon::parse($cliente->data_nascimento)->format('d/m/Y') }}</span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-2">
                            <strong>Endereço:</strong> {{ optional($cliente->endereco)->endereco ?? 'N/A' }},
                            {{ optional($cliente->endereco)->numero ?? 'N/A' }}
                        </div>
                        <div class="col-md-4 mb-2">
                            <strong>Bairro:</strong> {{ optional($cliente->endereco)->bairro ?? 'N/A' }}
                        </div>
                        <div class="col-md-4 mb-2">
                            <strong>Cidade:</strong> {{ optional($cliente->endereco)->cidade ?? 'N/A' }}
                        </div>
                        <div class="col-md-4 mb-2">
                            <strong>Estado:</strong> {{ optional($cliente->endereco)->estado ?? 'N/A' }}
                        </div>
                        <div class="col-md-4 mb-2">
                            <strong>CEP:</strong>
                            {{ $cliente->endereco && $cliente->endereco->cep ? \Illuminate\Support\Str::substr($cliente->endereco->cep, 0, 5) . '-' . \Illuminate\Support\Str::substr($cliente->endereco->cep, 5) : 'N/A' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if ($cliente->vendas->count() > 0)
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('relatorios.vendas', ['cliente_id' => $cliente->id]) }}"
                                ...
                                <tbody>
                                    @foreach ($cliente->vendas as $venda)
                                        <tr>
                                            ...
                                            <td><strong>R$ {{ number_format($venda->valor_total, 2, ',', '.') }}</strong>
                                            </td>
                                            <td>
            ...
            @if ($cliente->orcamentos->count() > 0)
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('orcamentos.index', ['search' => $cliente->nome]) }}"
                                ...
                                <tbody>
                                    @foreach ($cliente->orcamentos as $orcamento)
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="badge bg-{{ $orcamento->status == 'autorizado' ? 'success' : ($orcamento->status == 'recusado' ? 'danger' : 'warning') }}">
            ...
            @if ($cliente->ordensServico->count() > 0)
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('os.index', ['search' => $cliente->nome]) }}" class="btn btn-sm btn-primary">Ver
                            Todas</a>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                                ...
                                <tbody>
                                    @foreach ($cliente->ordensServico as $os)
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="badge bg-{{ $os->status == 'concluida' ? 'success' : ($os->status == 'cancelada' ? 'danger' : 'warning') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <p><strong>Cadastrado em:</strong><br>
                        <small
                            class="text-muted">{{ \Carbon\Carbon::parse($cliente->created_at)->format('d/m/Y H:i') }}</small>
                    </p>
                    <p><strong>Última atualização:</strong><br>
                        <small
                            class="text-muted">{{ \Carbon\Carbon::parse($cliente->updated_at)->format('d/m/Y H:i') }}</small>
                    </p>
                </div>
            </div>
        </div>
