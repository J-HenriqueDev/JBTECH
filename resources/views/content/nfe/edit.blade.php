            ...
            @if (in_array($notaFiscal->status, ['digitacao', 'pendente', 'rejeitada', 'erro']))
                <form action="{{ route('nfe.transmitir', $notaFiscal->id) }}" method="POST" class="d-grid d-md-inline">
                    ...
                    <button type="submit" class="btn btn-success btn-sm"
                        onclick="return confirm('Deseja transmitir esta NF-e para a SEFAZ?')">
            ...
            @if ($notaFiscal->status != 'autorizada' && $notaFiscal->status != 'cancelada')
                <form action="{{ route('nfe.destroy', $notaFiscal->id) }}" method="POST" class="d-grid d-md-inline">
                    ...
                    <button type="submit" class="btn btn-danger btn-sm"
                        onclick="return confirm('Tem certeza que deseja excluir esta NF-e?')">
    ...
    <form action="{{ route('nfe.update', $notaFiscal->id) }}" method="POST">
        ...
        <div class="row g-3">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="d-flex align-items-center">
                                    <span
                                        class="badge bg-{{ $badgeColor }} me-2">{{ ucfirst($notaFiscal->status) }}</span>
                            ...
                            <div class="col-md-2">
                                ...
                                <div>
                                    {{ $notaFiscal->data_emissao ? $notaFiscal->data_emissao->format('d/m/Y H:i:s') : 'N/A' }}
                                </div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="small font-monospace user-select-all bg-light p-1 rounded">
                                    {{ $notaFiscal->chave_acesso ?? 'Pendente' }}</div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="text-primary fw-bold fs-5">R$
                                    {{ number_format((float) $notaFiscal->valor_total, 2, ',', '.') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body pt-3">
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    <label class="form-label text-muted fw-bold small text-uppercase">Inscrição
                                        Estadual</label>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label text-muted fw-bold small text-uppercase">Município /
                                        UF</label>
                                    <div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <select name="pagamento[forma]" class="form-select form-select-sm">
                                    <option value="01"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '01' ? 'selected' : '' }}>
                                        Dinheiro</option>
                                    <option value="03"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '03' ? 'selected' : '' }}>
                                        Cartão de Crédito</option>
                                    <option value="04"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '04' ? 'selected' : '' }}>
                                        Cartão de Débito</option>
                                    <option value="15"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '15' ? 'selected' : '' }}>
                                        Boleto Bancário</option>
                                    <option value="90"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '90' ? 'selected' : '' }}>Sem
                                        Pagamento</option>
                                    <option value="99"
                                        {{ ($notaFiscal->dados_pagamento['forma'] ?? '') == '99' ? 'selected' : '' }}>
                                        Outros</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Indicador</label>
                                <select name="pagamento[indicador]" class="form-select form-select-sm"
                                    id="pagamento_indicador">
                                    <option value="0"
                                        {{ ($notaFiscal->dados_pagamento['indicador'] ?? '') == '0' ? 'selected' : '' }}>À
                                        Vista</option>
                                    <option value="1"
                                        {{ ($notaFiscal->dados_pagamento['indicador'] ?? '') == '1' ? 'selected' : '' }}>À
                                        Prazo</option>
                                </select>
                            </div>
                            <div class="col-md-4" id="div_qtd_parcelas"
                                style="{{ ($notaFiscal->dados_pagamento['indicador'] ?? '') == '1' ? '' : 'display: none;' }}">
                                ...
                                <input type="number" name="pagamento[qtd_parcelas]" class="form-control form-control-sm"
                                    value="{{ $notaFiscal->dados_pagamento['qtd_parcelas'] ?? '' }}" min="1"
                                    max="60" placeholder="Ex: 3">
                            </div>
                        </div>
                        @php
                                        ...
                                        <tbody>
                                            @foreach ($parcelas as $index => $parcela)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="date"
                                                            name="pagamento[parcelas][{{ $index }}][data]"
            ...
            <div class="col-12">
                <div class="card mb-3">
                    <div
                        class="card-header bg-light py-2 d-flex flex-wrap justify-content-between align-items-center gap-2">
                            ...
                            <table class="table table-bordered table-hover table-sm align-middle" id="table-produtos">
                                ...
                                <tbody>
                                    @forelse($produtos as $index => $produto)
                                        @php
                                            ...
                                            $valorTotal =
                                                $produto['vProd'] ??
                                                ($produto['valor_total'] ?? $quantidade * $valorUnitario);
                                        @endphp
                                        <tr>
                                            <td>
                                                <input type="text" name="produtos[{{ $index }}][cProd]"
                                                    class="form-control form-control-sm"
                                            ...
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <button type="button" class="btn btn-outline-secondary"
                                                        onclick="buscarNCM('ncm_nfe_{{ $index }}')">
                                                        <i class="bx bx-search"></i>
                                                    </button>
                                                </div>
                                            </td>
                                            <td>
                                                <input type="text" name="produtos[{{ $index }}][CFOP]"
                                                    class="form-control form-control-sm"
                                                    value="{{ $produto['CFOP'] ?? '' }}">
                                            </td>
                                            <td>
                                                <input type="text" name="produtos[{{ $index }}][uCom]"
                                            ...
                                            <td class="text-end">
                                                ...
                                                <span class="valor-total"
                                                    data-index="{{ $index }}">{{ number_format((float) $valorTotal, 2, ',', '.') }}</span>
                                                <input type="hidden" name="produtos[{{ $index }}][vProd]"
                                                    class="valor-total-input" data-index="{{ $index }}"
                                ...
                                <tfoot>
                                    <tr class="table-light">
                                        ...
                                        <td><strong>R$ <span
                                                    id="total-nota-valor">{{ number_format((float) $notaFiscal->valor_total, 2, ',', '.') }}</span></strong>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body pt-3">
                        <div class="mb-3">
                            ...
                            <small class="form-text text-muted">Essas informações serão usadas no campo de Informações
                                Complementares da NF-e.</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-stretch gap-2">
                    ...
                    @if (!$isReadOnly)
                        ...
                        <button type="submit" name="action" value="transmitir" class="btn btn-success btn-sm"
                            onclick="return confirm('Deseja salvar e transmitir esta NF-e para a SEFAZ?')">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select id="modal_produto_id" class="form-select select2" style="width: 100%">
                                    ...
                                    @foreach ($produtosDisponiveis as $prod)
                                        <option value="{{ $prod->id }}" data-nome="{{ $prod->nome }}"
                                            data-preco="{{ $prod->preco_venda }}"
                                            data-unidade="{{ $prod->unidade_comercial }}"
                                            data-ncm="{{ $prod->ncm }}">
                                            {{ $prod->nome }} (R$
                                            {{ number_format((float) $prod->preco_venda, 2, ',', '.') }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <input type="number" id="modal_quantidade" class="form-control" min="0.0001"
                                    step="0.0001" value="1">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Valor Unitário</label>
                                <input type="number" id="modal_valor_unitario" class="form-control" min="0.01"
                        ...
                        <form action="{{ route('nfe.cancelar', $notaFiscal->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="mb-2">
                                        <span class="d-block mb-1 small text-muted">Selecione um motivo rápido ou
                                            personalize abaixo:</span>
                            ...
                            <div class="modal-body">
                                <div class="alert alert-info">
                                    <small>A Carta de Correção é disciplinada pelo § 1º-A do art. 7º do Convênio S/N, de 15
                                        de dezembro de
                                        1970 e pode ser utilizada para regularização de erro ocorrido na emissão de
                                        documento fiscal, desde que o
                                        erro não esteja relacionado com: I - as variáveis que determinam o valor do imposto
                                        tais como: base de
                                        cálculo, alíquota, diferença de preço, quantidade, valor da operação ou da
                                        prestação; II - a correção de
                                        dados cadastrais que implique mudança do remetente ou do destinatário; III - a data
                                        de emissão ou de
                                        saída.</small>
                                </div>
                                <div class="mb-3">
