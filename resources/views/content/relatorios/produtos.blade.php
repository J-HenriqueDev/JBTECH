...
@section('content')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="estoque_minimo" class="form-control"
                            value="{{ request('estoque_minimo', isset($estoqueMinimoPadrao) ? $estoqueMinimoPadrao : '') }}"
                            placeholder="Ex: 10">
                    </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h5>Valor Total em Estoque: <strong class="text-success">R$
                    {{ number_format($valorTotalEstoque ?? 0, 2, ',', '.') }}</strong></h5>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse($produtos as $produto)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge bg-{{ $produto->estoque <= 10 ? 'danger' : ($produto->estoque <= 50 ? 'warning' : 'success') }}">
                                        {{ $produto->estoque ?? 0 }}
                                    </span>
                                </td>
