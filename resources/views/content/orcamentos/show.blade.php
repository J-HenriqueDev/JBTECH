...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="mb-0 text-primary"
            ...
            <span
                class="badge bg-{{ $orcamento->status == 'autorizado' ? 'success' : ($orcamento->status == 'recusado' ? 'danger' : ($orcamento->status == 'apagado' ? 'secondary' : 'warning')) }} ms-2">
    ...
    @if (!$podeAutorizar && $orcamento->status == 'pendente')
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            ...
            <p class="mb-2">Não é possível autorizar este orçamento. Os seguintes produtos não possuem estoque suficiente:
            </p>
            <ul class="mb-0">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <span
                                class="text-muted {{ $orcamento->isVencido() && $orcamento->status == 'pendente' ? 'text-danger' : '' }}">
                                    ...
                                    @foreach ($formas as $forma)
                                        <li>
                                            ...
                                            @if ($forma == 'boleto' && $orcamento->parcelas_boleto)
                                                <span class="text-muted">({{ $orcamento->parcelas_boleto }}x @if ($orcamento->periodicidade_boleto)
                                                        - {{ $orcamento->periodicidade_boleto }}
                                                    @endif)</span>
                                            @endif
                                        </li>
                                    @endforeach
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <p class="mb-0">
                            ...
                            {{ $orcamento->cliente->endereco->cidade }}/{{ $orcamento->cliente->endereco->estado }}<br>
                            CEP:
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($orcamento->produtos as $produto)
                                    <tr>
                                        ...
                                        <td>R$ {{ number_format((float) $produto->pivot->valor_unitario, 2, ',', '.') }}
                                        </td>
                                        <td><strong>R$
                                                {{ number_format((float) $produto->pivot->valor_total, 2, ',', '.') }}</strong>
                                        </td>
                                        <td>
                                            <span
                                                class="badge bg-{{ $produto->estoque < $produto->pivot->quantidade ? 'danger' : ($produto->estoque <= 10 ? 'warning' : 'success') }}">
                                                {{ $produto->estoque }}
                                            </span>
                                            @if ($produto->estoque < $produto->pivot->quantidade)
                                                <small class="text-danger d-block">Faltam
                                                    {{ $produto->pivot->quantidade - $produto->estoque }} unidades</small>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td><strong class="text-success" style="font-size: 1.1rem;">R$
                                            {{ number_format($orcamento->valor_total, 2, ',', '.') }}</strong></td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        @if ($orcamento->status == 'pendente')
                            <form id="formAutorizar" action="{{ route('orcamentos.autorizar', $orcamento->id) }}"
                                method="POST">
                                @csrf
                                @if (!$podeAutorizar)
                                    ...
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name="ignorar_estoque"
                                            id="ignorar_estoque" checked>
                            ...
                            <form id="formRecusar" action="{{ route('orcamentos.recusar', $orcamento->id) }}"
                                method="POST">
                                @csrf
                        ...
                        <a href="{{ route('orcamentos.gerarPdf', $orcamento->id) }}" class="btn btn-danger"
                            target="_blank">
                            <i class="fas fa-file-pdf"></i> Gerar PDF
                        </a>
                        @if ($orcamento->status == 'autorizado')
                            <a href="{{ route('vendas.index', ['cliente_id' => $orcamento->cliente_id]) }}"
                                class="btn btn-info">
            ...
            <div class="modal-content">
                <form id="formAutorizar" action="{{ route('orcamentos.autorizar', $orcamento->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>Tem certeza que deseja autorizar este orçamento? <strong>Uma venda será criada
                        ...
                        @if (!$podeAutorizar)
                            ...
                            <div class="form-check mb-3 bg-light p-2 rounded border">
                                <input class="form-check-input" type="checkbox" name="adicionar_compras"
                                    id="adicionar_compras" checked>
                                <label class="form-check-label fw-bold" for="adicionar_compras">
                                    Adicionar itens faltantes à Lista de Compras?
                                </label>
                                <small class="d-block text-muted ms-1">Criará uma solicitação de compra automática para os
                                    itens sem estoque.</small>
                            </div>
                        @endif
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger"
                        onclick="document.getElementById('formRecusar').submit()">Sim, Recusar</button>
                </div>
            </div>
        </div>
    </div>
