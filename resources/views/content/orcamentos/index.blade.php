            ...
            <div class="col-md-3">
                <div class="card bg-warning text-white">
                    <div class="card-body">
                        ...
                        <small class="d-block mt-1">R$
                            {{ number_format($stats['valor_total_pendente'], 2, ',', '.') }}</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card bg-success text-white">
                    <div class="card-body">
                        ...
                        <small class="d-block mt-1">R$
                            {{ number_format($stats['valor_total_autorizado'], 2, ',', '.') }}</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="dropdown">
                                <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownFiltros"
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownFiltros">
                                    ...
                                    <li>
                                        <div class="dropdown-item">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="ordenacao"
                                                    id="ordenacaoRecentes" value="recentes"
                                                    {{ request('ordenacao', 'recentes') == 'recentes' ? 'checked' : '' }}
                                                    onchange="aplicarFiltros()">
                                                <label class="form-check-label" for="ordenacaoRecentes">Mais recentes
                                                    primeiro</label>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="dropdown-item">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="ordenacao"
                                                    id="ordenacaoAntigos" value="antigos"
                                                    {{ request('ordenacao') == 'antigos' ? 'checked' : '' }}
                                                    onchange="aplicarFiltros()">
                                                <label class="form-check-label" for="ordenacaoAntigos">Mais antigos
                                                    primeiro</label>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="dropdown-item">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="ordenacao"
                                                    id="ordenacaoMaiorValor" value="maior_valor"
                                                    {{ request('ordenacao') == 'maior_valor' ? 'checked' : '' }}
                                                    onchange="aplicarFiltros()">
                                                <label class="form-check-label" for="ordenacaoMaiorValor">Maior valor
                                                    primeiro</label>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="dropdown-item">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="ordenacao"
                                                    id="ordenacaoMenorValor" value="menor_valor"
                                                    {{ request('ordenacao') == 'menor_valor' ? 'checked' : '' }}
                                                    onchange="aplicarFiltros()">
                                                <label class="form-check-label" for="ordenacaoMenorValor">Menor valor
                                                    primeiro</label>
                                            </div>
                                        </div>
                                    </li>
                ...
                <div class="card-body">
                    <!-- Seção de Filtros -->
                    <div class="collapse {{ request('search') || request('status') || request('data_inicio') || request('data_fim') ? 'show' : '' }} mb-4"
                        id="collapseFiltros">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" name="search" id="search" class="form-control"
                                            placeholder="ID, Cliente ou CPF/CNPJ" value="{{ request('search') }}">
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="status[]" class="form-select" multiple size="4">
                                            <option value="pendente"
                                                {{ in_array('pendente', request('status', [])) ? 'selected' : '' }}>
                                                Pendente
                                            </option>
                                            <option value="autorizado"
                                                {{ in_array('autorizado', request('status', [])) ? 'selected' : '' }}>
                                                Autorizado</option>
                                            <option value="recusado"
                                                {{ in_array('recusado', request('status', [])) ? 'selected' : '' }}>
                                                Recusado
                                            </option>
                                            <option value="apagado"
                                                {{ in_array('apagado', request('status', [])) ? 'selected' : '' }}>Apagado
                                            </option>
                                        </select>
                                    ...
                                    <div class="mt-3 pt-3 border-top">
                                        ...
                                        <div class="d-flex flex-wrap gap-2">
                                            @foreach ($statusSelecionados as $status)
                                                ...
                                                @endphp
                                                <span
                                                    class="badge bg-{{ $status == 'autorizado' ? 'success' : ($status == 'recusado' ? 'danger' : ($status == 'apagado' ? 'secondary' : 'warning')) }} d-inline-flex align-items-center">
                                                    {{ ucfirst($status) }}
                                                    <a href="{{ route('orcamentos.index', $novoRequest) }}"
                                                        class="text-white ms-2"
                                                        style="text-decoration: none; font-size: 1.1em; line-height: 1;"
                                                        title="Remover filtro">×</a>
                                                </span>
                                            @endforeach
                                        </div>
                                    </div>
                            ...
                            <tbody>
                                @foreach ($orcamentos as $orcamento)
                                    <tr>
                                        ...
                                        <td class="orcamento-data">
                                            {{ \Carbon\Carbon::parse($orcamento->data)->format('d/m/Y') }}</td>
                                        ...
                                        <td class="orcamento-valor"><strong>R$
                                                {{ number_format((float) $orcamento->valor_total, 2, ',', '.') }}</strong>
                                        </td>
                                        <td class="orcamento-status">
                                            <span
                                                class="badge bg-{{ $orcamento->status == 'autorizado' ? 'success' : ($orcamento->status == 'recusado' ? 'danger' : ($orcamento->status == 'apagado' ? 'secondary' : 'warning')) }}">
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                ...
                                                <a href="{{ route('orcamentos.gerarPdf', $orcamento->id) }}"
                                                    target="_blank" class="btn btn-sm btn-icon btn-outline-secondary"
                                                    title="Gerar PDF">
                                                    <i class="bx bx-file"></i>
                                                </a>
                                                <button type="button" class="btn btn-sm btn-icon btn-outline-danger"
                                                    onclick="confirmDelete('{{ route('orcamentos.destroy', $orcamento->id) }}')"
                                                    title="Excluir">
