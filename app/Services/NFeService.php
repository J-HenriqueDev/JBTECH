...
class NFeService
{
  ...
  /**
   * Manifesta ciência, confirmação ou desconhecimento da operação
   */
  public function manifestar($chave, $codEvento)
  {
    try {
      if (!$this->tools) {
        $this->loadCertificate();
      ...
      $resp = $this->tools->sefazManifesta($chave, $codEvento);

      $st = new Standardize();
      $std = $st->toStd($resp);

      if (isset($std->cStat) && in_array($std->cStat, ['128', '135', '136', '573'])) {
        return true;
      }

      $motivo = $std->xMotivo ?? 'Erro desconhecido na manifestação';
      throw new Exception("SEFAZ retornou erro: {$std->cStat} - {$motivo}");
    } catch (\Exception $e) {
      Log::error("Erro ao manifestar chave {$chave}: " . $e->getMessage());
      throw $e;
    }
  }
  ...
  protected function processarCompleta($xml, $nsu)
  {
    ...
    // --- INICIO DA RECUPERAÇÃO DA LÓGICA DE DUPLICATAS E MARKUP ---
    try {
      // 1. Processamento de Fornecedor e Contas a Pagar (Duplicatas)
      ...
      if ($infNFe) {
        ...
        if (!ContaPagar::where('nota_entrada_id', $nota->id)->exists()) {
          ...
          if ($cobr && isset($cobr->dup)) {
            ...
            Log::info("Financeiro gerado automaticamente para Nota {$nota->id}");
          }
        }

        // 2. Processamento de Itens e Markup (Atualização de Preços)
        if (isset($infNFe->det)) {
          $detalhes = is_array($infNFe->det) ? $infNFe->det : [$infNFe->det];
          foreach ($detalhes as $det) {
            $prod = $det->prod;
            $ean = (string) $prod->cEAN;
            $cProd = (string) $prod->cProd;
            $novoCusto = (float) $prod->vUnCom;

            // Busca Produto (Lógica padrão de prioridade)
            $produto = Produto::where('codigo_barras', $cProd)->first();
            if (!$produto && $ean && $ean !== 'SEM GTIN') {
              $produto = Produto::where('codigo_barras', $ean)->first();
            }

            if ($produto) {
              $custoAntigo = $produto->preco_custo;
              $produto->preco_custo = $novoCusto;

              // Lógica de Markup: Mantém a margem atual ou aplica padrão se custo era zero
              if ($custoAntigo > 0 && $produto->preco_venda > 0) {
                $margemAtual = ($produto->preco_venda / $custoAntigo); // Ex: 1.5 para 50%
                $produto->preco_venda = $novoCusto * $margemAtual;
              } elseif ($produto->preco_venda == 0) {
                // Se não tinha preço de venda, aplica markup padrão de 30%
                $produto->preco_venda = $novoCusto * 1.30;
              }

              $produto->save();
              Log::info("Markup atualizado para Produto ID {$produto->id}: Custo {$custoAntigo} -> {$novoCusto}");
            }
          }
        }
      }
    } catch (\Exception $e) {
      Log::error("Erro na lógica recuperada de Financeiro/Markup para Nota {$nota->id}: " . $e->getMessage());
      // Não interrompe o fluxo principal, apenas loga o erro da parte "extra"
    }
    // --- FIM DA LÓGICA RECUPERADA ---
