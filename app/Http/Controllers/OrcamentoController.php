...
class OrcamentoController extends Controller
{
    ...
    public function autorizar(Request $request, $id)
    {
        ...
        try {
            ...
            // Verifica se há estoque suficiente antes de autorizar (a menos que seja forçado)
            if (!$request->has('ignorar_estoque') && !$orcamento->podeSerAutorizado()) {
                $produtosSemEstoque = [];
                foreach ($orcamento->produtos as $produto) {
                    // Ignorar serviços
                    if ($produto->tipo_item === '09') {
            ...
            // REMOVIDO: Cobrança não é mais criada na autorização.
            // O pagamento será definido na finalização da venda.
            /*
            Cobranca::create([
                'venda_id' => $venda->id,
            ...
            Log::info('Cobrança criada para venda:', ['venda_id' => $venda->id]);
            */

            // Verificar itens faltantes e criar solicitação de compra se solicitado
            if ($request->has('adicionar_compras') && $request->adicionar_compras == 'on') {
                $itemsParaCompra = [];
                foreach ($orcamento->produtos as $produto) {
                    // Não adicionar serviços à lista de compras
                    if ($produto->tipo_item === '09') {
                        continue;
                    }

                    $estoqueDisponivel = max(0, $produto->estoque);
                    $quantidadeSolicitada = $produto->pivot->quantidade;

                    if ($estoqueDisponivel < $quantidadeSolicitada) {
                        $itemsParaCompra[] = [
                            'produto_id' => $produto->id,
                            'quantidade' => $quantidadeSolicitada - $estoqueDisponivel,
                            'valor_unitario' => 0 // Valor estimado a ser preenchido na compra
                        ];
                    }
                }

                if (!empty($itemsParaCompra)) {
                    $compra = \App\Models\Compra::create([
                        'user_id' => Auth::id(),
                        'cliente_id' => $orcamento->cliente_id, // Vincula ao cliente do orçamento
                        'tipo' => 'reposicao',
                        'prioridade' => 'alta', // Prioridade alta pois é venda travada/pendente
                        'status' => 'solicitado',
                        'data_compra' => now(),
                        'observacoes' => "Gerado automaticamente na autorização do Orçamento #{$orcamento->id} para o cliente {$orcamento->cliente->nome}.",
                    ]);

                    foreach ($itemsParaCompra as $item) {
                    ...
                    // Adiciona mensagem de sucesso sobre a compra
                    // session()->flash('success_compra', "Solicitação de compra #{$compra->id} criada para os itens faltantes.");
                }
            }
